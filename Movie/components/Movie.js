...
export const getNowPlayingList = async () => {
  const response = await axios.get(`${API_URL}/now_playing?api_key=${API_KEY}&language=ko-KR&page=1`);
  return response.data;
};

//popularList api
export const getPopularList = async () => {
  const response = await axios.get(`${API_URL}/popular?api_key=${API_KEY}&language=ko-KR&page=1`);
  return response.data;
};
